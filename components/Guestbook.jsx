...
const Guestbook = () => {
  ...
  return (
    ...
    <Button className='bg-teal-500' onClick={handleSignMessage}>Sign my Guestbook</Button>
  <div className='flex gap-8 flex-wrap'> {entries&&entries.map((entry)=>{return <div className='flex gap-2 rounded-md m-2 bg-[whitesmoke] w-fit p-2 text-black'><Identicon bg="whitesmoke" size={25} string={entry.signer} /><Text>{entry.signer.substring(0,6)+"..."+entry.signer.substring(entry.signer.length-4,entry.signer.length)}</Text></div>})}</div>
    </VStack>
    </div>
  )
}
